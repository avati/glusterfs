    		      ...
    		      void *xattrargs)
{
	...
	if (!strcmp (key, GF_CONTENT_KEY)
            && IA_ISREG (filler->stbuf->ia_type)) {
                ...
                /* File is snapshotted, don't fill in anything */
                if (!inode_ctx_get (filler->loc->inode, THIS, NULL))
                        goto err;
    			 ...
    			 dict_t *xattr_req, struct iatt *buf)
{
        ...
        if (!inode_ctx_get (loc->inode, this, NULL)) {
                filler.real_path = alloca (strlen (real_path) + 16);
                strcpy ((char *)filler.real_path, (char *)real_path);
                strcat ((char *)filler.real_path, "/HEAD/data");
        } else {
              ...
              loc_t *loc, dict_t *xattr_req)
{
	...
	VALIDATE_OR_GOTO (loc->path, out);
        ...
        if (!snap_entry) {
                snap_entry = gf_check_and_change_snap_entry (this, real_path,
                                                             &buf, xattr_req);
        }
            ...
            loc_t *loc)
{
        ...
        /* TODO: Isn't checking just the 'inode->ctx' for snapshot
           file enough? */
        op_ret = gf_check_if_snap_path (this, loc, real_path, &buf,
                                        NULL, NULL);
        if (!op_ret) {
               ...
               loc_t *loc, struct iatt *stbuf, int32_t valid)
{
        ...
        /* TODO: If the operation is on snapshot, then we need to handle
           it properly */

        op_ret = posix_lstat_with_gfid (this, real_path, &statpre);
        if (op_ret == -1) {
                op_ret = gf_check_if_snap_path (this, loc, real_path, &statpre,
                                                NULL, NULL);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
              ...
              loc_t *loc)
{
        ...
        snap_str = strstr (real_path, priv->snap_cmd_symbol);
        if (snap_str) {
                /* Snapshot to be deleted */
                memcpy (temp_path, real_path, (snap_str - real_path));
                op_ret = gf_snap_delete_snapshot (this, loc, temp_path,
                                                  (snap_str +
                                                   strlen (priv->snap_cmd_symbol)));
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
                                "snapshot delete on %s: %s", real_path,
                                strerror (op_errno));
                        goto out;
                }
                goto post_op;
        ...
        /* Snapshot file */
        if (gf_is_a_snapshot_file (this, real_path)) {
                gf_snap_delete_full_path (this, real_path);
                goto post_op;
        }
              ...
              loc_t *oldloc, loc_t *newloc)
{
        ...
        char *newsnap_str = NULL;
        char *oldsnap_str = NULL;
        char *from        = NULL;
        char *to          = NULL;
        char  temp_path1[ZR_PATH_MAX];
        char  temp_path2[ZR_PATH_MAX];
        ...
        /* TODO: check if snapshot rename is the command */
        oldsnap_str = strstr (real_oldpath, priv->snap_cmd_symbol);
        newsnap_str = strstr (real_newpath, priv->snap_cmd_symbol);
        if (oldsnap_str && newsnap_str) {
                memcpy (temp_path1, real_oldpath, (oldsnap_str - real_oldpath));
                memcpy (temp_path2, real_newpath, (newsnap_str - real_newpath));
                if (strcmp (temp_path1, temp_path2) == 0) {
                        /* snapshots of same file */
                        from = oldsnap_str + strlen (priv->snap_cmd_symbol);
                        to   = newsnap_str + strlen (priv->snap_cmd_symbol);
                        op_ret = gf_snap_rename (this, temp_path1, from, to);
                        if (op_ret) {
                                op_errno = errno;
                                gf_log (this->name, GF_LOG_ERROR,
                                        "%s: renaming snapshot %s -> %s failed: %s",
                                        temp_path1, from, to, strerror (op_errno));
                                goto out;
                        }
                        goto post_op;
                }
        ...
        if (op_ret == -1) {
                op_errno = errno;
                if (op_errno == ENOTDIR) {
                        /* Possible that source is snapshotted file,
                           destination is not */
                        gf_log (this->name, GF_LOG_INFO,
                                "%s (snapshot) -> %s (regular file)",
                                real_oldpath, real_newpath);
                        op_ret = unlink (real_newpath);
                        if (op_ret) {
                                gf_log (this->name, GF_LOG_ERROR,
                                        "failed to unlink %s", real_newpath);
                                goto out;
                        }
                        op_ret = rename (real_oldpath, real_newpath);
                        if (op_ret) {
                ...
                if (op_errno == EISDIR) {
                        /* Source is a file, destination is snapshotted file */
                        gf_log (this->name, GF_LOG_INFO,
                                "%s (regular file) -> %s (snapshot)",
                                real_oldpath, real_newpath);
                        op_ret = gf_snap_delete_full_path (this, real_newpath);
                        if (op_ret) {
                                gf_log (this->name, GF_LOG_ERROR,
                                        "failed to unlink %s", real_newpath);
                                goto out;
            ...
            loc_t *oldloc, loc_t *newloc)
{
        ...
        char                 *snap_str                 = NULL;
        char                  temp_path[ZR_PATH_MAX]   = {0,};
        char                  actual_file[ZR_PATH_MAX] = {0,};
        char                 *snap_name                = NULL;
        ...
        snap_str = strstr (real_oldpath, priv->snap_cmd_symbol);
        if (snap_str) {
                memcpy (temp_path, real_oldpath, (snap_str - real_oldpath));
                op_ret = gf_snap_create_clone (this, temp_path,
                                               (snap_str +
                                                strlen (priv->snap_cmd_symbol)),
                                               real_newpath);
                if (op_ret == -1) {
        ...
        if (gf_is_a_snapshot_file (this, real_oldpath) &&
            !strstr (real_newpath, priv->snap_cmd_symbol)) {
                /* Snapshotted file */
                op_ret = gf_create_another_snapshot (this, oldloc, real_oldpath,
                                                     newloc->name, NULL);
                if (op_ret) {
                        gf_log (this->name, GF_LOG_ERROR,
                                "failed to create snapshot %s on %s",
                                newloc->name, oldloc->path);
                        goto out;
                }
                op_ret = gf_snap_create_clone (this, real_oldpath, newloc->name,
                                               real_newpath);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        snap_name = strstr (newloc->name, priv->snap_cmd_symbol);
        if (snap_name) {
                memcpy (actual_file, real_newpath,
                        (strstr (real_newpath, priv->snap_cmd_symbol) - real_newpath));
                op_ret = posix_lstat_with_gfid (this, actual_file, &stbuf);
                if (op_ret == -1)
                        goto regular_link;

                if (IA_ISDIR (stbuf.ia_type)) {
                        /* already a snapshot or a valid directory */
                        if (!gf_is_a_snapshot_file (this, actual_file)) {
                                goto regular_link;
                        }
                        if (strstr (snap_name, "list") ||
                            strstr (snap_name, "HEAD")) {
                                op_ret = -1;
                                op_ret = EPERM;
                                goto out;
                        }

                        op_ret = gf_create_another_snapshot (this, oldloc, actual_file,
                                                             (snap_name +
                                                              strlen (priv->snap_cmd_symbol)),
                                                             NULL);
                        if (op_ret)
                        ...
                        op_ret = -1;
                        op_errno = EBUSY; /* Success */
                        goto out;
                }

                /* File exists, snapshot do able */
                op_ret = gf_create_fresh_snapshot (this, oldloc, actual_file,
                                                   (snap_name +
                                                    strlen (priv->snap_cmd_symbol)),
                                                   NULL);
                if (op_ret)
                ...
                off_t offset)
{
	...
	uint64_t              tmp_pfd = 0;
        ...
        SET_FS_ID (frame->root->uid, frame->root->gid);
        MAKE_REAL_PATH (real_path, this, loc->path);
        /* TODO: snapshot, check inode->ctx instead */
        if (sys_lgetxattr (real_path, GF_SNAP_FILE_KEY, NULL, 0) > 0) {
                snap_file = 1;
                strcat (real_path, "/HEAD/data");
        }
            ...
            loc_t *loc, int32_t flags, fd_t *fd, int wbflags)
{
        ...
        if (op_ret == -1) {
                ...
                if (snap_str) {
                        memcpy (temp_path, real_path, (snap_str - real_path));
                        _fd = gf_snap_open_snapshot (this, pfd, temp_path,
                                                     (snap_str + strlen (priv->snap_cmd_symbol)),
                                                     flags);
                        if (_fd == -1) {
                                op_ret   = -1;
                                op_errno = errno;
                                gf_log (this->name, GF_LOG_ERROR,
                                        "snapshot open on %s: %s", real_path,
                                        strerror (op_errno));
                                goto out;
                        }
                        goto post_op;
        ...
        if (gf_is_a_snapshot_file (this, real_path)) {
                /* Snapshotted file */
                _fd = gf_snap_open_snapshot (this, pfd, real_path, NULL, flags);
                if (_fd == -1) {
                        op_ret   = -1;
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
                ...
                loc_t *loc, const char *name)
{
	...
	if (loc->inode && IA_ISREG (loc->inode->ia_type) && name &&
        ...
        /* Snapshot: check if its the 'virtual file' */
        if (strstr (real_path, priv->snap_cmd_symbol))
                goto done;
                  ...
                  fd_t *fd, size_t size, off_t off, int whichop)
{
	...
	pfd = (struct posix_fd *)(long)tmp_pfd;
        ...
        strncpy (entry_path, real_path, real_path_len);
...
#endif
        tmp_data = dict_get (this->options, "snapshot-command");
        if (tmp_data) {
                gf_log (this->name, GF_LOG_INFO,
                        "using '%s' as the snapshot command", tmp_data->data);
                _private->snap_cmd_symbol = gf_strdup (tmp_data->data);;
        } else {
                _private->snap_cmd_symbol = gf_strdup ("@@@");;
        }
