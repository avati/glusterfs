...
#define MAKE_INODE_HANDLE(rpath, this, loc, iatt_p) do {                \
        ...
        /* expand ELOOP */                                              \
        __path = alloca (PATH_MAX);                                     \
        op_ret = posix_handle_expand (this, __path, loc->gfid, NULL);   \
        if (op_ret == -1)                                               \
                break;                                                  \
        rpath = __path;                                                 \
        op_ret = posix_pstat (this, loc->gfid, rpath, iatt_p);          \
...
#define MAKE_ENTRY_HANDLE(entp, parp, this, loc, ent_p) do {            \
        ...
        /* expand ELOOP */                                              \
        entp = alloca (PATH_MAX);                                       \
        op_ret = posix_handle_expand (this, entp, loc->pargfid,         \
                                      loc->name);                       \
        if (op_ret == -1)                                               \
                break;                                                  \
        __parp = strdupa (entp);                                        \
        parp = dirname (__parp);                                        \
        posix_pstat (this, NULL, entp, ent_p);                          \
        } while (0)
...
int
posix_handle_expand (xlator_t *this, char *path, uuid_t gfid, const char *base);
